        ...
        return $.extend({
            create: function (container, editParm, controlOptions)
            {
                ...
                if (field.editor)
                {
                    ...
                    ext = typeof (ext) == 'function' ? ext(editParm, inputBody) : ext;
                    $.extend(p, ext);
                }
                //返回的是ligerui对象
        ...
        "date": {
            ...
            setValue: function (editor, value, editParm)
            {
                ...
                if(value){
                	...
                	value = editor.getFormatDate(new Date(value));
                }
                editor.setValue(value);
            }
        },
...
(function ($)
{
    ...
    $.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            {
				// edit by xuli
                var header = $('<div class="l-accordion-header" id="'+ box.attributes.ligerid.value +'"><div class="l-accordion-toggle"></div><div class="l-accordion-header-inner"></div></div>');
                if (i == selectedIndex)
...
(function ($)
{
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            var g = this, p = this.options;
            if (date == "NaN" || date == null) return null;
            var format = p.format;
            var o = {
                "M+": date.getMonth() + 1,
                "d+": date.getDate(),
        ...
        _getValue: function ()
        {
        	return this.getFormatDate(this.usedDate);
        },
        setEnabled: function ()
        {
            var g = this, p = this.options;
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
       ...
       /* //删除分组 
        ...
        addRule: function (jgroup)
        {
			...
			// edit by xuli
            //事件：删除条件
            $("#" + $(this).attr('id') + " .deleterole").live('click', function ()
            {
                var rulerow = $(this).parent().parent();
                g.deleteRule(rulerow);
            });
...
(function ($)
{
    ...
    // edit by xuli
    $.ligerDefaults.Form.editors.label =
    {
        create: function (container, editParm, p)
        {
            var editor = $('<label />');
    ...
    $.ligerDefaults.Form.editors.file =
    {
        ...
        {
        	var paths = '';
        	$("#"+editParm.field.name).find("input").each(function(i,cb){        		
        		paths += $(cb).val() + '|';
        	});
    ...
    $.ligerDefaults.Form.editors.html =
    {
        ...
        {       
             ...
             editor.append(editParm.field.html);
             container.append(editor);
             return editor;
        },
        getValue: function (editor, editParm)
        {
			var result = {};
			$("#"+editParm.field.name).find("input").each(function(i,cb){ 
				result[$(cb).context.id] = $(cb).context.checked;
			});		
			return result;
        },
        setValue: function (editor, value, editParm)
        {
            $("#"+editParm.field.name).find("input").each(function(i,cb){        		
            	$(cb).attr(editParm.field.htmlattr, value[i]);
            });
        }
    };
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
            ...
            function getFieldValueToData(fields, tabIndex)
            {
                $(fields).each(function (fieldIndex, field)
                {
                    ...
                    if (name)
                    {
                        ...
                        if(field.type == 'file'){
                        	var value = editor.editor.getValue(editor.control, {
                                field: {name : name + '-removevalue'}
                            });
                            g._setValueByName(g.data, 'remove_'+ name, value);
                        }
                    }
                    if (textField)
                    {
                    ...
                    if(field.type == 'date')
                    {
                    	g.data[name] = $("input[name='"+ name +"']").val();
                    }
                    if(field.isOrgControl){
                    	var ids = $("#" + name).val();
                    	var names = $("#" + name + "_name").val();
                    	if(ids && names && ids.substr(ids.length -1, ids.length) == ','){
                    		if(ids){
                    	...
                    	g._setValueByName(g.data, name, ids);
                    	g._setValueByName(g.data, name + "_name", names);
                    }
                });
            }
            return g.data;
        ...
        valid: function ()
        {
            ...
            var flag =  g.form.valid();  
            var autocompleteFlag = true; 
            // edit by xuli 验证autocomplete
            for(var i in p.fields){
            	var id = $("input[name=" + p.fields[i].name+"]").attr("id");
            	if(id && $("#" + id).attr("autocomplete") && p.fields[i].validate){
            		if(p.fields[i].validate.required && ($("#" + $("#" + id).attr("hiddenid")).val() == undefined  || $("#" + $("#" + id).attr("hiddenid")).val().length == 0)){
            			this.showFieldError(p.fields[i].name, "请选择" + p.fields[i].display + "！");
            			autocompleteFlag = false;
            			break;
            		}
            	}
            }
            if(flag && autocompleteFlag){
            	return true;
            }else{
            	return false;
            }
        ...
        {
        	// edit by xuli
            var element = $("[ligeruiid=" + name + "]", this.element);
            var areaElement = $("[name=" + name + "]", this.element);
            if (element.hasClass("l-textarea"))
            {
                element.removeClass("l-textarea-invalid");
            }
            else if(areaElement.hasClass("l-textarea")){
            	areaElement.removeClass("l-textarea-invalid");
            }
            else if(element.parent().hasClass("l-text"))
            {
                element.parent().removeClass("l-text-invalid");
            }
            else if(areaElement.parent().hasClass("l-text"))
            {
            	areaElement.parent().removeClass("l-text-invalid");
            }
            // edit by xuli
            $(element).removeAttr("title").ligerHideTip();
            $(areaElement).removeAttr("title").ligerHideTip();           
        },
        hideAllFieldError: function ()
        {
        	p = this.options;
        	for ( var i = 0, l = p.fields.length; i < l; i++ )
            {
        ...
        setFieldValidate: function (name, validate, messages)
        {
            ...
            if (oldRule) //旧的验证移除验证结果
            {
                ...
                // edit by xuli
                if (jele.parents("li:first").next("li:first").find(".l-star").hasClass('l-star'))//旧的验证包括必填规则，移除*
                {
            ...
            if (validate.required || validate.requiredFlag)
            {
            	// edit by xuli 删除标红验证
            	if(validate.requiredFlag){
            		delete validate.requiredFlag;
            	}
                //验证包括必填规则，添加*
        ...
        _buliderSpaceContainer: function (field)
        {
			...
			// edit by xuli
            if(field.afterLabel != undefined && field.afterLabel.length > 0){
            	out.push("<label>" + field.afterLabel + "</label>");
            }
            out.push('</li>');
            return out.join('');
        },
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [50, 100, 200],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
		...
		// edit by xuli        
		contentType: "application/json; charset=utf-8",                     // Ajax
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm)
        {
			...
			// edit by xuli
            var param = {};
            if(p.contentType.indexOf('json') != -1){
        ...
        */
        ...
        _initColumns: function ()
        {
            ...
            $(g.columns).each(function (i, column)
            {
				...
				column.isExport = column.isExport == false ? false : true;
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
				...
				var classStr = null;
				var style = null;		
				// edit by xuli 修改行rend方式
                if (p.rowAttrRender){ 
                	var result = p.rowAttrRender(item, rowid);
					if(result && result.classStr){
                ...
                if (style) gridhtmlarr.push(style);
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            else if (text != null)
            {
				...
				// edit by xuli
                if(content != null) content = content.toString().replace("<", "&lt;").replace('>', '&gt;');
            }
            else if (value != null)
            {
            ...
            if (src.checkboxall) //复选框全选
            {
                ...
                for (var rowid in g.records)
                {
                    if (uncheck)
                    	...
                    	//grid复选框全选时，不选中禁用的行。edit by 袁宏洲
                    	if($(g.getRowObj(rowid,true)).find('span').hasClass("l-grid-row-cell-btn-checkbox")||$(g.getRowObj(rowid)).find('span').hasClass("l-grid-row-cell-btn-checkbox"))
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addTabItem: function (options)
        {
            ...
            // 根据url获取id
			// edit by xuli
            var utabid = g.getTabIdByUrl(url);
            if(utabid){
            	tabid = utabid;
            }
            //如果已经存在
		...
		// edit by xuli
        reload: function (tabid, _url)
        {
            var g = this, p = this.options;
            var contentitem = $(".l-tab-content-item[tabid=" + tabid + "]");
            var iframeloading = $(".l-tab-loading:first", contentitem);
            var iframe = $("iframe:first", contentitem);
            var url = _url || $(iframe).attr("src");
            iframeloading.show();
		...
		// edit by xuli
        getTabIdByName : function(name){
        	var g = this, p = this.options;
        	return $("li", g.tab.links.ul).find("a:contains('"+ name +"')").parent().attr("tabid") || '';
        },
        getTabIdByUrl : function(url){
        	var g = this, p = this.options;
        	var result = '';
        	var _url;
        	if(url){
        		_url = url.substr(url.indexOf('/'));
        		if(_url.indexOf('?') != -1){
        			_url = _url.substr(0, _url.lastIndexOf('?') - 1);
        		}
    		}
        	$.each(g.tab.content.find('iframe'), function(i, cb){
        		if($(cb).attr('src').indexOf(_url) != -1){
        			result = $(cb).attr('id');
        		}
        	})
        	return result;
...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getNodes: function ()
        {
        	var g = this, p = this.options;
            var nodes = [];
            $(".l-note", g.tree).parent().parent("li").each(function ()
            {
                var treedataindex = parseInt($(this).attr("treedataindex"));
                nodes.push({ target: this, data: g._getDataNodeByTreeDataIndex(g.data, treedataindex) });
            });
        ...
        refreshTree: function () {
            var g = this, p = this.options;
			$.each(this.getNodes2(),function(k,v){		
        ...
        _setTreeEven: function ()
        {
            ...
            g.tree.click(function (e)
            {
                ...
                g.trigger('click', [{ data: treenodedata, target: treeitem[0] , 'obj' : obj}]);
            });
